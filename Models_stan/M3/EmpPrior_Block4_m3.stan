data {
  ...
  real<lower=-1,upper=1> rew[NS,MT];//subject x trial reward
  int choice[NS,MT];//chosen option
  int choice_two[NS,MT];//1=chose left,0=chose right
  real stickiness[NS,MT];//1=previous choice left, -1=previous choice right
}
...
model {
  ...
  alpha[:,1] ~ beta(1.15,1.84);
  alpha[:,2] ~ beta(1.01,1.20);
  beta[:,1] ~ normal(0.050,0.63);
  beta[:,2] ~ normal(2.55,3.18);
  beta[:,3] ~ normal(-0.13,2.8);
      ...
      if (choice[s,t] > 0) {
        if (t < 76) {
          choice_two[s,t] ~ bernoulli_logit(beta[s,1] +
                                            beta[s,2]*(Q[s,t,stim[s,t],1]-Q[s,t,stim[s,t],2]) +
